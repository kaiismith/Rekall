...
> **AI notes (Kat).** While a meeting is in progress, the **Kat** live-notes
> assistant produces a running summary + key points + open questions every
> ~20 seconds against a sliding window of finalized transcript segments. Notes
> are generated by an Azure AI Foundry chat-completions deployment using
> either an API key (set `KAT_FOUNDRY_API_KEY`) or `DefaultAzureCredential`
> (leave the key empty in production — managed identity / `az login` /
> workload identity all work). Kat notes are **NOT persisted** — they live
> only in process memory for the duration of the meeting and are dropped
> when the last participant leaves or the backend restarts. The transcript
> itself remains the durable record. Smoke checklist:
> [`backend/docs/smoke-kat-live-notes.md`](backend/docs/smoke-kat-live-notes.md).
